        ...
        if (pauseCtx->namedItem != PAUSE_ITEM_NONE) {
            if ((pauseCtx->pageIndex == PAUSE_MAP) && !sInDungeonScene) {
                ...
                const char* textureName =
                    OoTxMm_GetSpecialItemNameTexture(pauseCtx->namedItem, gSaveContext.language, NULL);
...
void KaleidoScope_Draw(PlayState* play) {
    ...
    __gSPSegment(POLY_OPA_DISP++, 0x07, VIRTUAL_TO_PHYSICAL(pauseCtx->playerSegment));

    ...
    if (canCycle && slotItem != ITEM_NONE && (showLeftItem || showRightItem)) {
        ...
        if (showLeftItem) {
            ...
            KaleidoScope_DrawQuadTextureRGBA32(play->state.gfxCtx,
                                               OoTxMm_GetSpecialItemIconTexture(leftItem, gItemIcons[leftItem]), 32, 32,
                                               0);
        ...
        if (showRightItem) {
            ...
            KaleidoScope_DrawQuadTextureRGBA32(play->state.gfxCtx,
                                               OoTxMm_GetSpecialItemIconTexture(rightItem, gItemIcons[rightItem]), 32,
                                               32, 4);
            gSPGrayscale(POLY_OPA_DISP++, false);
        }
...
void KaleidoScope_HandleItemCycles(PlayState* play) {
    ...
    KaleidoScope_HandleItemCycleExtras(
        ...
        (IS_RANDO && !OoTxMm_IsMaskSelectItem(INV_CONTENT(ITEM_TRADE_CHILD))) ? Randomizer_GetPrevChildTradeItem()
                                                                              : OoTxMm_GetOwnedMaskCyclePrevItem(INV_CONTENT(ITEM_TRADE_CHILD)),
        (IS_RANDO && !OoTxMm_IsMaskSelectItem(INV_CONTENT(ITEM_TRADE_CHILD))) ? Randomizer_GetNextChildTradeItem()
                                                                              : OoTxMm_GetOwnedMaskCycleNextItem(INV_CONTENT(ITEM_TRADE_CHILD)),
        true);
    ...
    gSlotAgeReqs[SLOT_TRADE_CHILD] =
        (CVarGetInteger(CVAR_ENHANCEMENT("AdultMasks"), 0) || CVarGetInteger(CVAR_CHEAT("TimelessEquipment"), 0)) &&
                OoTxMm_IsMaskSelectItem(INV_CONTENT(ITEM_TRADE_CHILD))
            ? AGE_REQ_NONE
            : AGE_REQ_CHILD;

    // also update the age requirements for the masks itself
    for (int i = 0; i < 10; i += 1) {
        gItemAgeReqs[OoTxMm_GetMaskCycleItem(i)] =
            CVarGetInteger(CVAR_ENHANCEMENT("AdultMasks"), 0) || CVarGetInteger(CVAR_CHEAT("TimelessEquipment"), 0)
                ? AGE_REQ_NONE
                : AGE_REQ_CHILD;
    }
...
void KaleidoScope_DrawItemSelect(PlayState* play) {
    ...
    for (i = j = 0; i < 24; i++, j += 4) {
        ...
        if (gSaveContext.inventory.items[i] != ITEM_NONE) {
            ...
            KaleidoScope_DrawQuadTextureRGBA32(play->state.gfxCtx,
                                               OoTxMm_GetSpecialItemIconTexture(itemId, gItemIcons[itemId]), 32, 32, 0);
            gSPGrayscale(POLY_OPA_DISP++, false);
        }
    }

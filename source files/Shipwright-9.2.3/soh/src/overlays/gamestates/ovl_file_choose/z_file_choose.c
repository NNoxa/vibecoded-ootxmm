...
void FileChoose_UpdateQuestMenu(GameState* thisx) {
    ...
    if (false && (ABS(this->stickRelX) > 30 || (dpad && CHECK_BTN_ANY(input->press.button, BTN_DLEFT | BTN_DRIGHT)))) {
...
void FileChoose_UpdateRandomizerMenu(GameState* thisx) {
    ...
    if (CHECK_BTN_ALL(input->press.button, BTN_A)) {
        if (this->randomizerIndex == RSM_START_RANDOMIZER) {
            ...
            Audio_PlaySoundGeneral(NA_SE_SY_FSEL_DECIDE_L, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                                   &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            SohFileSelect_ShowComboRandomizerMenu();
        }
    }
}
...
void FileChoose_Init(GameState* thisx) {
    ...
    s32 pad;
    this->logoAlpha = 0;
    this->questType[0] = QUEST_OOTXMM;
    this->questType[1] = QUEST_OOTXMM;
    this->questType[2] = QUEST_OOTXMM;

...
static s32 BgSpot06Objects_LockWasHitByZoraBoomerang(BgSpot06Objects* this, PlayState* play) {
    Actor* actor = play->actorCtx.actorLists[ACTORCAT_MISC].head;
    Sphere16* lockSphere = &this->collider.elements[0].dim.worldSphere;
    Vec3f lockCenter = {
        lockSphere->center.x,
        lockSphere->center.y,
        lockSphere->center.z,
    ...
    while (actor != NULL) {
        if (actor->id == ACTOR_EN_BOOM && (actor->params == 1 || actor->params == 2) &&
            Math_Vec3f_DistXYZ(&actor->world.pos, &lockCenter) <= lockSphere->radius + 20.0f) {
            actor->world.pos = actor->prevPos;
            return true;
        }
        actor = actor->next;
    }
...
void BgSpot06Objects_LockWait(BgSpot06Objects* this, PlayState* play) {
    ...
    if ((this->collider.base.acFlags & AC_HIT) || BgSpot06Objects_LockWasHitByZoraBoomerang(this, play)) {
        this->timer = 130;

...
void EnBoom_Fly(EnBoom* this, PlayState* play) {
    ...
    if ((this->actor.params == 1 || this->actor.params == 2) && this->returnTimer == 1 &&
        ...
        this->actor.world.rot.y = player->actor.shape.rot.y + (this->actor.params == 1 ? -0x4000 : 0x4000);
    }
...
void EnBoom_Draw(Actor* thisx, PlayState* play) {
    ...
    zoraFinDL = OoTxMm_GetZoraBoomerangDisplayList(this->actor.params);
    if (zoraFinDL != NULL) {
    ...
    if (zoraFinDL != NULL) {
        gSPDisplayList(POLY_OPA_DISP++, (Gfx*)zoraFinDL);
    } else {

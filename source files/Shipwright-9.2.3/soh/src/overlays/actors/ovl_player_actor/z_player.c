...
#include "soh/Enhancements/transform_masks/TransformMasks.h"
...
static s32 (*sItemActionUpdateFuncs[])(Player* this, PlayState* play) = {
    ...
    func_8083485C,                 // PLAYER_IA_MASK_DEKU
    func_8083485C,                 // PLAYER_IA_MASK_FIERCE_DEITY
...
void Player_PlayVoiceSfx(Player* this, u16 sfxId) {
    if (OoTxMm_PlayTransformMaskVoiceSfx(this, sfxId)) {
        return;
    }

    if (((sfxId == NA_SE_VO_LI_AUTO_JUMP) && OoTxMm_ShouldSuppressJumpSfx(this)) ||
        OoTxMm_ShouldSuppressLinkVoiceSfx(this, sfxId)) {
        return;
    }

    if (this->actor.category == ACTORCAT_PLAYER) {
        Player_PlaySfx(this, sfxId + (OoTxMm_IsFierceDeitySkinMode(this) ? 0 : this->ageProperties->unk_92));
    } else {
...
void Player_PlaySteppingSfx(Player* this, f32 pitchAdjustment) {
    ...
    if (OoTxMm_PlayTransformMaskStepSfx(this, pitchAdjustment)) {
...
void Player_PlayJumpingSfx(Player* this) {
    ...
    if (OoTxMm_ShouldSuppressJumpSfx(this)) {
        return;
    }
...
s8 Player_ItemToItemAction(s32 item) {
    ...
    } else if (item == ITEM_MASK_DEKU) {
        return PLAYER_IA_MASK_DEKU;
    } else if (item == ITEM_MASK_FIERCE_DEITY) {
        return PLAYER_IA_MASK_FIERCE_DEITY;
    } else if (item >= ARRAY_COUNT(sItemActions)) {
        return PLAYER_IA_NONE;
    } else {
...
void Player_ProcessItemButtons(Player* this, PlayState* play) {
    ...
    if (this->currentMask != PLAYER_MASK_NONE && !CVarGetInteger(CVAR_ENHANCEMENT("PersistentMasks"), 0)) {
        maskItemAction = Player_ItemToItemAction(OoTxMm_PlayerMaskToItemId(this->currentMask));
...
s32 func_80834BD4(Player* this, PlayState* play) {
    ...
    this->stateFlags1 |= PLAYER_STATE1_SHIELDING;
    if (!OoTxMm_IsFierceDeitySkinMode(this)) {
        Player_SetModelsForHoldingShield(this);
    }
...
void OoTxMm_PlayerStartZoraBoomerangPipeline(Player* this, PlayState* play) {
    if (this == NULL || play == NULL) {
        return;
    }
    ...
    this->stateFlags1 |= PLAYER_STATE1_USING_BOOMERANG;
    this->stateFlags1 &= ~(PLAYER_STATE1_ITEM_IN_HAND | PLAYER_STATE1_READY_TO_FIRE | PLAYER_STATE1_FIRST_PERSON);
...
void OoTxMm_PlayerSetZoraBoomerangHeld(Player* this, s32 held) {
    if (this == NULL) {
        return;
    }

    if (OoTxMm_IsZoraBoomerangAiming(this)) {
        sUseHeldItem = held;
        sHeldItemButtonIsHeldDown = held;
    }
}

void OoTxMm_PlayerStartZoraBoomerangAimCamera(Player* this, PlayState* play) {
    if (this == NULL || play == NULL) {
        return;
    }
...
void OoTxMm_PlayerCleanupZoraBoomerangPipeline(Player* this) {
    if (this == NULL) {
        return;
    }
    ...
    this->unk_6AD = 0;
    this->unk_834 = 0;
    this->stateFlags1 &= ~(PLAYER_STATE1_USING_BOOMERANG | PLAYER_STATE1_ITEM_IN_HAND | PLAYER_STATE1_READY_TO_FIRE);
...
void Player_UseItem(PlayState* play, Player* this, s32 item) {
    ...
    if (((this->heldItemAction == this->itemAction) &&
        ...
        if ((itemAction == PLAYER_IA_NONE) || !(this->stateFlags1 & PLAYER_STATE1_IN_WATER) ||
            ...
            if ((play->bombchuBowlingStatus == 0) &&
            ...
            } else if ((itemAction >= PLAYER_IA_MASK_KEATON) && (itemAction <= PLAYER_IA_MASK_FIERCE_DEITY)) {
                ...
                osSyncPrintf("[OoTxMM Transform] Equipped mask action=%d currentMask=%d\n", itemAction, this->currentMask);
...
s32 Player_GetMovementSpeedAndYaw(Player* this, f32* outSpeedTarget, s16* outYawTarget, f32 speedMode,
    ...
    if (!Player_CalcSpeedAndYawFromControlStick(play, this, outSpeedTarget, outYawTarget, speedMode)) {
        ...
        *outSpeedTarget *= OoTxMm_GetTransformMaskMovementSpeedMultiplier(this);
        return true;
    }
}
...
void func_80837948(PlayState* play, Player* this, s32 arg2) {
    ...
    u32 dmgFlags;
    s32 temp;
    LinkAnimationHeader* transformJumpSlashAnim;
    ...
    transformJumpSlashAnim = OoTxMm_GetZoraJumpSlashAnim(this, arg2);
    Player_AnimPlayOnceAdjusted(play, this,
                                transformJumpSlashAnim != NULL ? transformJumpSlashAnim : D_80854190[arg2].unk_00);
    ...
    if (OoTxMm_IsFierceDeitySkinMode(this) && gSaveContext.magic > 0 &&
        arg2 >= PLAYER_MWA_FORWARD_SLASH_1H && arg2 < PLAYER_MWA_FLIPSLASH_START) {
        ...
        Actor* beam = Actor_Spawn(&play->actorCtx, play, ACTOR_EN_M_THUNDER,
                                  this->bodyPartsPos[PLAYER_BODYPART_WAIST].x,
                                  this->bodyPartsPos[PLAYER_BODYPART_WAIST].y,
                                  this->bodyPartsPos[PLAYER_BODYPART_WAIST].z, beamPitch, 0, 0, 0x80);
        if (beam != NULL) {
            gSaveContext.magic--;
    ...
    if (this->unk_A86 != 0) {
        ...
        if (sp68 || (this->actor.bgCheckFlags & 0x100) || (sFloorType == 9) ||
            ...
            if (this->cylinder.base.acFlags & AC_HIT) {
                ...
                if (OoTxMm_IsTransformMaskFireImmune(this)) {
                    return 0;
                }
...
s32 Player_ActionHandler_12(Player* this, PlayState* play) {
    ...
    f32 wallPolyNormalZ;
    f32 sp24;
    f32 transformLedgeBonus = OoTxMm_GetTransformMaskLedgeReachBonus(this);

    if (OoTxMm_ShouldBlockLedgeClimb(this)) {
        return 0;
    }

    if (!(this->stateFlags1 & PLAYER_STATE1_CARRYING_ACTOR) && (this->ledgeClimbType >= 2) &&
        (!(this->stateFlags1 & PLAYER_STATE1_IN_WATER) ||
         ((this->ageProperties->unk_14 + transformLedgeBonus) > this->yDistToLedge))) {
        sp3C = 0;

        if (func_808332B8(this)) {
            if (this->actor.yDistToWater < 50.0f) {
                if ((this->ledgeClimbType < 2) || (this->yDistToLedge > (this->ageProperties->unk_10 + transformLedgeBonus))) {
                    return 0;
                }
            } else if ((this->currentBoots != PLAYER_BOOTS_IRON) || (this->ledgeClimbType > 2)) {
                return 0;
            }
        } else if (!(this->actor.bgCheckFlags & 1) || (((this->ageProperties->unk_14 + transformLedgeBonus) <= this->yDistToLedge) &&
                                                       (this->stateFlags1 & PLAYER_STATE1_IN_WATER))) {
            return 0;
        }
        ...
        if (sp3C != 0) {
            ...
            if ((this->ageProperties->unk_14 + transformLedgeBonus) <= sp34) {
                ...
                if (func_808332B8(this)) {
                    ...
                    this->stateFlags1 &= ~PLAYER_STATE1_IN_WATER;
                } else if ((this->ageProperties->unk_18 + transformLedgeBonus) <= sp34) {
...
s32 func_8083A6AC(Player* this, PlayState* play) {
    ...
    if ((this->actor.yDistToWater < -80.0f) && (ABS(this->floorPitch) < 2730) && (ABS(this->floorPitchAlt) < 2730)) {
        ...
        if (BgCheck_EntityLineTest1(&play->colCtx, &this->actor.world.pos, &sp74, &sp68, &sp84, true, false, false,
            ...
            if (!sp50 && OoTxMm_ShouldBlockLedgeClimb(this)) {
                return 0;
            }
        ...
        if (controlStickDirection <= PLAYER_STICK_DIR_FORWARD) {
            if (Player_IsZTargeting(this)) {
                if (this->actor.category != ACTORCAT_PLAYER) {
                    ...
                    if (OoTxMm_ShouldForceZoraJumpSlash(this)) {
...
s32 Player_ActionHandler_11(Player* this, PlayState* play) {
    LinkAnimationHeader* anim;
    f32 frame;
    s32 fdSwordBlock = OoTxMm_IsFierceDeitySkinMode(this) && (Player_GetMeleeWeaponHeld(this) != 0);

    if ((play->shootingGalleryStatus == 0) && ((this->currentShield != PLAYER_SHIELD_NONE) || fdSwordBlock) &&
        CHECK_BTN_ALL(sControlInput->cur.button, BTN_R) &&
        (Player_IsChildWithHylianShield(this) ||
         fdSwordBlock || (!Player_FriendlyLockOnOrParallel(this) && (this->focusActor == NULL)))) {
        ...
        if (this->actor.velocity.y > 0.0f) {
            if (this->actor.yDistToWater < this->ageProperties->unk_30) {
                ...
                if (func_8083CFA8(play, this, this->actor.velocity.y, 500) &&
                    !OoTxMm_PlayTransformMaskGeneralSfx(this, NA_SE_PL_FACE_UP)) {
            ...
            if (this->actor.velocity.y < 0.0f) {
                if (this->av2.actionVar2 >= 0) {
                    ...
                    if (!GameInteractor_GetDisableLedgeGrabsActive() && (this->actor.bgCheckFlags & 0x200) &&
                        ...
                        (this->linearVelocity > 0.0f)) {
                        if ((this->yDistToLedge >= (150.0f + OoTxMm_GetTransformMaskLedgeReachBonus(this))) &&
                            (this->controlStickDirections[this->controlStickDataIndex] == 0)) {
                            func_8083EC18(this, play, sTouchedWallFlags);
                        } else if ((this->ledgeClimbType >= 2) &&
                                   (this->yDistToLedge < (150.0f + OoTxMm_GetTransformMaskLedgeReachBonus(this))) &&
...
void Player_Action_80845668(Player* this, PlayState* play) {
    ...
    if (this->skelAnime.animation == &gPlayerAnim_link_normal_250jump_start) {
        ...
        if (LinkAnimation_OnFrame(&this->skelAnime, 8.0f)) {
            ...
            if (temp1 > (this->ageProperties->unk_0C + OoTxMm_GetTransformMaskLedgeCheckHeightBonus(this))) {
                temp1 = this->ageProperties->unk_0C + OoTxMm_GetTransformMaskLedgeCheckHeightBonus(this);
            }
...
#define vLedgeCheckOffsetXZ float0
                ...
                ledgeCheckPos.y = this->actor.world.pos.y + this->ageProperties->unk_0C +
                                  OoTxMm_GetTransformMaskLedgeCheckHeightBonus(this);
                ...
                if ((this->yDistToLedge < 18.0f) ||
                    ...
                    if (Player_PosVsWallLineTest(play, this, &sInteractWallCheckOffset, &poly, &bgId,
                        ...
                        if ((this->ageProperties->unk_1C + OoTxMm_GetTransformMaskLedgeReachBonus(this)) <=
                            this->yDistToLedge) {
                            if (ABS(ledgeFloorPoly->normal.y) > 28000) {
                                if ((this->ageProperties->unk_14 + OoTxMm_GetTransformMaskLedgeReachBonus(this)) <=
                                    this->yDistToLedge) {
                                    nextLedgeClimbType = 4;
                                } else if ((this->ageProperties->unk_18 + OoTxMm_GetTransformMaskLedgeReachBonus(this)) <=
                                           this->yDistToLedge) {
...
void Player_UpdateCommon(Player* this, PlayState* play, Input* input) {
    ...
    if (this->stateFlags2 & PLAYER_STATE2_PAUSE_MOST_UPDATING) {
        ...
        OoTxMm_UpdateTransformMaskState(play, this);

        Player_UpdateCamAndSeqModes(play, this);
        OoTxMm_UpdateZoraBoomerangAimCamera(play, this);
        ...
        if (OoTxMm_IsTransformationMaskPlayerMask(this->currentMask)) {
            this->cylinder.dim.height = OoTxMm_GetTransformMaskColliderHeight(this);
            this->cylinder.dim.radius = OoTxMm_GetTransformMaskColliderRadius(this);
            this->cylinder.dim.yShift = OoTxMm_GetTransformMaskColliderYShift(this);
        } else {
            this->cylinder.dim.height = this->cylinder.dim.height * OoTxMm_GetTransformMaskColliderHeightScale(this);
            if (this->cylinder.dim.height < OoTxMm_GetTransformMaskColliderMinHeight(this)) {
                this->cylinder.dim.height = OoTxMm_GetTransformMaskColliderMinHeight(this);
            }

            this->cylinder.dim.radius = D_80854624.dim.radius * OoTxMm_GetTransformMaskColliderRadiusScale(this);
            if (this->cylinder.dim.radius < OoTxMm_GetTransformMaskColliderMinRadius(this)) {
                this->cylinder.dim.radius = OoTxMm_GetTransformMaskColliderMinRadius(this);
            }
            this->cylinder.dim.yShift += OoTxMm_GetTransformMaskColliderYShiftOffset(this);
        }
...
void Player_Update(Actor* thisx, PlayState* play) {
    ...
    if (Player_UpdateNoclip(this, play)) {
        ...
        if (this->stateFlags1 & (PLAYER_STATE1_INPUT_DISABLED | PLAYER_STATE1_IN_CUTSCENE)) {
            ...
            {
                u16 transformMaskBlockedButtons = OoTxMm_GetTransformMaskInputBlockMask(play, this);
                if (transformMaskBlockedButtons != 0) {
                    sp44.cur.button &= ~transformMaskBlockedButtons;
                    sp44.press.button &= ~transformMaskBlockedButtons;
                }
            }
        }
...
void Player_DrawGameplay(PlayState* play, Player* this, s32 lod, Gfx* cullDList, OverrideLimbDrawOpa overrideLimbDraw) {
    ...
    bool hideTransformMaskPlayer = OoTxMm_ShouldHideTransformMaskPlayerDraw(this);
    if (hideTransformMaskPlayer) {
        // First-person view is positioned at the form's head, so drawing the form clips the camera inside it.
    } else if (OoTxMm_ShouldOverridePlayerDraw(this) && !OoTxMm_ShouldSuspendOverride(this)) {
        void* transformEyes = OoTxMm_GetTransformMaskEyeTexture(this);
        void* transformMouth = OoTxMm_GetTransformMaskMouthTexture(this);

        if (transformEyes != NULL) {
            gSPSegment(POLY_OPA_DISP++, 0x08, (uintptr_t)SEGMENTED_TO_VIRTUAL(transformEyes));
        }

        if (transformMouth != NULL) {
            gSPSegment(POLY_OPA_DISP++, 0x09, (uintptr_t)SEGMENTED_TO_VIRTUAL(transformMouth));
        }

        OoTxMm_DrawTransformMaskPlayer(play, this);
    } else {
    ...
    OoTxMm_DrawTransformMaskOverlays(play, this);

    if (!hideTransformMaskPlayer && !OoTxMm_ShouldOverridePlayerDraw(this) && (overrideLimbDraw == Player_OverrideLimbDrawGameplayDefault) &&
        (this->currentMask != PLAYER_MASK_NONE)) {
        ...
        if ((this->currentMask != PLAYER_MASK_BUNNY || !CVarGetInteger(CVAR_ENHANCEMENT("HideBunnyHood"), 0)) &&
            sMaskDlists[this->currentMask - 1] != NULL) {
...
s16 func_8084ABD8(PlayState* play, Player* this, s32 arg2, s16 arg3) {
    ...
    if (CVarGetInteger(CVAR_SETTING("MoveInFirstPerson"), 0) &&
        CVarGetInteger(CVAR_SETTING("Controls.RightStickAim"), 0) &&
        !OoTxMm_IsTransformationMaskPlayerMask(this->currentMask)) {
...
void Player_Action_8084B1D8(Player* this, PlayState* play) {
    ...
    bool transformZoraBoomerangAiming = OoTxMm_IsZoraBoomerangAiming(this);
    if ((this->csAction != 0) || (this->unk_6AD == 0) || (this->unk_6AD >= 4) || Player_UpdateHostileLockOn(this) ||
        ...
        (((this->unk_6AD == 2) && !transformZoraBoomerangAiming &&

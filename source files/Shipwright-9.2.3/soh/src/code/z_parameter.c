                ...
                if (interfaceCtx->restrictions.tradeItems != 0) {
                    for (i = 1; i < ARRAY_COUNT(gSaveContext.equips.buttonItems); i++) {
                        if ((CVarGetInteger(CVAR_ENHANCEMENT("MMBunnyHood"), BUNNY_HOOD_VANILLA) !=
                             BUNNY_HOOD_VANILLA) &&
                            OoTxMm_IsMaskSelectItem(gSaveContext.equips.buttonItems[i])) {
                            gSaveContext.buttonStatus[BUTTON_STATUS_INDEX(i)] = BTN_ENABLED;
...
void Interface_DrawItemIconTexture(PlayState* play, void* texture, s16 button) {
    ...
    if (button >= 0 && button < ARRAY_COUNT(gSaveContext.equips.buttonItems)) {
        texture = OoTxMm_GetSpecialItemIconTexture(gSaveContext.equips.buttonItems[button], texture);
    }

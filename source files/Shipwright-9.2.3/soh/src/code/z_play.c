...
void Play_Init(GameState* thisx) {
    ...
    if (CVarGetInteger(CVAR_ENHANCEMENT("AdultMasks"), 0) || CVarGetInteger(CVAR_CHEAT("TimelessEquipment"), 0)) {
        for (int i = 0; i < 10; i += 1) {
            gItemAgeReqs[OoTxMm_GetMaskCycleItem(i)] = AGE_REQ_NONE;
        }
        if (OoTxMm_IsMaskSelectItem(INV_CONTENT(ITEM_TRADE_CHILD))) {
            gSlotAgeReqs[SLOT_TRADE_CHILD] = AGE_REQ_NONE;
        }
    } else {
        for (int i = 0; i < 10; i += 1) {
            gItemAgeReqs[OoTxMm_GetMaskCycleItem(i)] = AGE_REQ_CHILD;
        }

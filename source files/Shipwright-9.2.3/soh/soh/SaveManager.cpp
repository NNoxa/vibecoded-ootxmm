...
void SaveManager::StartupCheckAndInitMeta(int fileNum) {
    ...
    if (!metaSaveBlock.contains("version")) {
        ...
        quarantineMalformedSave();
        return;
    }
    if (!metaSaveBlock.contains("sections") || !metaSaveBlock["sections"].contains("base") ||
        !metaSaveBlock["sections"]["base"].contains("data")) {
        SPDLOG_ERROR("Save at " + fileName.string() + " is missing the base section");
        quarantineMalformedSave();
        return;
    }
...
void SaveManager::LoadFile(int fileNum) {
    ...
    try {
        ...
        // Older randomizer saves only stored the broad file type. The base section's questId, when present, restores
        // more specific randomizer variants such as OoTxMM.
...
void SaveManager::LoadBaseVersion2() {
    ...
    int questId = gSaveContext.ship.quest.id;
    SaveManager::Instance->LoadData("questId", questId, questId);
    if (questId >= QUEST_NORMAL && questId <= QUEST_OOTXMM) {
        gSaveContext.ship.quest.id = questId;
    }
...
void SaveManager::SaveBase(SaveContext* saveContext, int sectionID, bool fullSave) {
    ...
    SaveManager::Instance->SaveData("questId", saveContext->ship.quest.id);

...
void GenerateRandomizerImgui(std::string seed = "") {
    ...
    if (IS_OOTXMM && ctx->IsSeedGenerated()) {
        SohCrossover::PublishOoTxMmSeed(ctx->GetSeedString(), ctx->GetSeed());
    }
...
bool GenerateRandomizer(std::string seed /*= ""*/) {
    ...
    if (CVarGetInteger(CVAR_GENERAL("RandoGenerating"), 0) == 0) {
        if (IS_OOTXMM) {
            SohGui::ApplyComboSettingsToSoH();
        }
...
extern "C" u16 Randomizer_Item_Give(PlayState* play, GetItemEntry giEntry) {
    ...
    if (SohCrossover::ConsumePendingCrossGameMmNativeRewardSuppression(giEntry)) {

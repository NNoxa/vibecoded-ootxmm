...
void BuildHintStoneMessage(uint16_t* textId, bool* loadFromMessageTable) {
    ...
    const std::string stoneKey = "oot:" + std::to_string(gPlayState->sceneNum) + ":" +
    ...
    if (const auto crossoverHint = SohCrossover::GetOoTxMmGossipHintMessage(stoneKey)) {
        msg = CustomMessage(*crossoverHint);
        msg.AutoFormat();
        msg.LoadIntoFont();

...
void BuildMerchantMessage(CustomMessage& msg, RandomizerCheck rc, bool mysterious = true) {
    ...
    const auto locationData = Rando::StaticData::GetLocation(rc);
    std::optional<std::string> crossoverName;
    if (locationData != nullptr) {
        crossoverName = SohCrossover::GetSharedPlacementDisplayNameByNativeCheck(locationData->GetName());
    }
    if (crossoverName.has_value()) {
        itemName = CustomMessage(*crossoverName);
        color = "%g";
    } else if (mysterious) {
        ...
        if (locationData != nullptr && locationData->IsShop()) {

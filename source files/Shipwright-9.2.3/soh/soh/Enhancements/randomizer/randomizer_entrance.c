...
static s8 Entrance_IsComboDungeonEntryway(s16 entranceIndex) {
    switch (entranceIndex) {
        ...
        case ENTR_INSIDE_GANONS_CASTLE_ENTRANCE:
            return CVarGetInteger("gRandomizer.Combo.Logic.DungeonEntrances", 0) >= 2;
        default:
            return 0;
    }
}

static s8 Entrance_ComboDungeonEntrancesEnabled(void) {
    return CVarGetInteger("gRandomizer.Combo.Logic.DungeonEntrances", 0) != 0;
}
...
void Entrance_Init(void) {
    ...
    for (size_t i = 0; i < ENTRANCE_OVERRIDES_MAX_COUNT; i++) {
        ...
        if (Entrance_ComboDungeonEntrancesEnabled() && Entrance_IsComboDungeonEntryway(originalIndex)) {
            continue;
        }
...
s16 Entrance_PeekNextIndexOverride(int16_t nextEntranceIndex) {
    if (Entrance_ComboDungeonEntrancesEnabled() && Entrance_IsComboDungeonEntryway(nextEntranceIndex)) {

...
std::string GetShopItemDisplayName(RandoItemId randoItemId, bool useItemArticle, RandoCheckId randoCheckId) {
    const auto staticCheck = Rando::StaticData::Checks.find(randoCheckId);
    if (staticCheck != Rando::StaticData::Checks.end()) {
        if (const auto crossoverName =
                TwoShipCrossover::GetSharedPlacementDisplayNameByNativeCheck(staticCheck->second.name)) {
            return *crossoverName;
        }
    }

    return Rando::StaticData::GetItemName(randoItemId, useItemArticle, randoCheckId);
}
...
void renameStolenBombBag(u16* textId, bool* loadFromMessageTable) {
    ...
    CustomMessage::Replace(
        &entry.msg, "{{itemName}}",
        GetShopItemDisplayName(randoSaveCheck.randoItemId, false, RC_BOMB_SHOP_ITEM_04_OR_CURIOSITY_SHOP_ITEM));
...
void renameSpecialBargain(u16* textId, bool* loadFromMessageTable) {
    ...
    CustomMessage::Replace(
        &entry.msg, "{{itemName}}",
        GetShopItemDisplayName(randoSaveCheck.randoItemId, false, RC_CURIOSITY_SHOP_SPECIAL_ITEM));
    CustomMessage::LoadCustomMessageIntoFont(entry);
    *loadFromMessageTable = false;
}
...
void Rando::ActorBehavior::InitEnGirlABehavior() {
    ...
    COND_ID_HOOK(OnOpenText, RANDO_DESC_TEXT_ID, IS_RANDO, [](u16* textId, bool* loadFromMessageTable) {
        ...
        CustomMessage::Replace(&entry.msg, "{{itemName}}",
                               GetShopItemDisplayName(randoSaveCheck.randoItemId, false, randoCheckId));
    ...
    COND_ID_HOOK(OnOpenText, 0x884, IS_RANDO, [](u16* textId, bool* loadFromMessageTable) {
        ...
        CustomMessage::Replace(&entry.msg, "{{itemName}}",
                               GetShopItemDisplayName(randoSaveCheck.randoItemId, true, randoCheckId));
    ...
    COND_ID_HOOK(OnOpenText, 0x648, IS_RANDO, [](u16* textId, bool* loadFromMessageTable) {
        ...
        CustomMessage::Replace(&entry.msg, "{{itemName}}",
                               GetShopItemDisplayName(randoSaveCheck.randoItemId, true,

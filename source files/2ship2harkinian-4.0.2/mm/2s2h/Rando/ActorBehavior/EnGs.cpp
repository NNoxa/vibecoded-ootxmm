...
bool sLastGossipWasCrossoverHint = false;
...
void Rando::ActorBehavior::InitEnGsBehavior() {
    ...
    COND_VB_SHOULD(VB_GS_CONTINUE_TEXTBOX, shouldRegister, {
        if (sLastGossipWasCrossoverHint) {
            sLastGossipWasCrossoverHint = false;
            return;
        }
    ...
    COND_ID_HOOK(OnOpenText, FIRST_GS_MESSAGE, shouldRegister, [](u16* textId, bool* loadFromMessageTable) {
        ...
        if (stone != nullptr && stone->id == ACTOR_EN_GS) {
            const std::string stoneKey = "mm:" + std::to_string(gPlayState->sceneId) + ":" +
                                         std::to_string(stone->params) + ":" +
                                         std::to_string(static_cast<int32_t>(stone->home.pos.x)) + ":" +
                                         std::to_string(static_cast<int32_t>(stone->home.pos.y)) + ":" +
                                         std::to_string(static_cast<int32_t>(stone->home.pos.z));
            if (const auto crossoverHint = TwoShipCrossover::GetOoTxMmGossipHintMessage(stoneKey)) {
                sLastGossipWasCrossoverHint = true;
                entry.autoFormat = false;
                entry.msg = *crossoverHint;
